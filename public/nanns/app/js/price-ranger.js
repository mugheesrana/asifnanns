...
$(document).ready(function () {
  ...
  // Detect home page price filter (uses #price-min / #price-max)
  ...
  var moneyFormat = wNumb({
    decimals: 0,
    thousand: ',',
    prefix: '' // show plain numbers without a unit prefix
  });

  var startMin = 0;
  var startMax = 1000000;
  var rangeMin = 0;
  var rangeMax = 1000000;

  if (priceMinInput && priceMaxInput) {
    var parsedMin = parseInt(priceMinInput.value, 10);
    var parsedMax = parseInt(priceMaxInput.value, 10);
  ...
  // Set visual min and max values and also update hidden form inputs
  rangeSlider.noUiSlider.on('update', function (values, handle) {
    var labelMin = document.getElementById('slider-range-value1');
    var labelMax = document.getElementById('slider-range-value2');

    if (labelMin) {
      labelMin.innerHTML = values[0];
    }
    if (labelMax) {
      labelMax.innerHTML = values[1];
    }

    var numericMin = moneyFormat.from(values[0]);
    var numericMax = moneyFormat.from(values[1]);

    if (priceMinInput && priceMaxInput) {
      priceMinInput.value = numericMin;
      priceMaxInput.value = numericMax;
    } else {
      // Fallback for legacy forms using min-value / max-value names
      var legacyMin = document.getElementsByName('min-value');
      var legacyMax = document.getElementsByName('max-value');
      if (legacyMin && legacyMin.length) {
        legacyMin[0].value = numericMin;
      }
      if (legacyMax && legacyMax.length) {
        legacyMax[0].value = numericMax;
      }
    }
  });
});
